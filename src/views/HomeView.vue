<script setup lang="ts">
  ...
  const availableYears = computed(() => {
    const years = Object.keys(statistics.value)
      .map((y) => parseInt(y))
      .sort((a, b) => b - a);
...
<template>
  ...
  <div v-else class="m-5">
    <div class="bg-surface-0 dark:bg-surface-800 rounded-xl shadow-lg border border-surface-200 dark:border-surface-700 p-6">
      ...
      <div v-if="!invoiceStore.loadingInvoices && Object.keys(statistics).length > 0" class="flex flex-wrap gap-3 mb-6 justify-center">
        <button
          ...
          :class="[
            ...
            visibleYears[year] !== false ? 'opacity-100 shadow-md hover:shadow-lg' : 'opacity-40 hover:opacity-60',
          ]"
          :style="{
            borderColor: yearColors[year]?.border || '#6b7280',
            backgroundColor: visibleYears[year] !== false ? yearColors[year]?.background || '#6b728080' : 'transparent',
            color: yearColors[year]?.border || '#6b7280',
          }"
        >
          <div class="w-4 h-4 rounded" :style="{ backgroundColor: yearColors[year]?.border || '#6b7280' }"></div>
      ...
      <!-- Wykres -->
      <div class="mt-4">
        <Chart v-if="chartData" type="line" :data="chartData" :options="chartOptions" style="height: 35vh; min-height: 350px" />
      </div>
    </div>
  </div>
  ...
  <div v-if="authorizationStore.isAuthenticatedOrToken" class="m-5 mt-6">
    <div class="bg-surface-0 dark:bg-surface-800 rounded-xl shadow-lg border border-surface-200 dark:border-surface-700 p-6">
      ...
      <div
        ...
        v-if="!invoiceStore.loadingInvoices && Object.keys(customerStatistics).length === 0"
      >
      ...
      <div
        v-if="!invoiceStore.loadingInvoices && Object.keys(customerStatistics).length > 0"
        class="flex flex-wrap gap-3 mb-6 justify-center"
      >
        <button
          ...
          :class="[
            'px-4 py-2 rounded-lg border-2 transition-all duration-200 flex items-center gap-2',
            visibleCustomers[customerId] !== false ? 'opacity-100 shadow-md hover:shadow-lg' : 'opacity-40 hover:opacity-60',
          ]"
          :style="{
            borderColor: getCustomerColor(customerId).border,
            backgroundColor: visibleCustomers[customerId] !== false ? getCustomerColor(customerId).background : 'transparent',
            color: getCustomerColor(customerId).border,
          }"
        >
          <div class="w-4 h-4 rounded" :style="{ backgroundColor: getCustomerColor(customerId).border }"></div>
