...
const isSaveBtnDisabled = computed(() => {
  return (
    costStore.loadingCosts ||
    supplierStore.loadingSupplier ||
...
const deleteConfirmationMessage = computed(() => {
  if (costItem.value)
    return `Czy chcesz usunąć pozycję <b>${costItem.value.name}</b>?`;
  return "No message";
});

const submitDelete = async () => {
  if (costItem.value) {
    if (costDeleteItemIndex.value !== -1)
      cost.value.costItems.splice(costDeleteItemIndex.value, 1);
  }
      ...
      <Panel>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Fieldset class="w-full" legend="Dane kosztu">
            ...
            <div
              class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:mt-4"
            >
          ...
          <Fieldset class="w-full" legend="Pozycje kosztu">
                  ...
                  <div class="text-center">
                    {{
                      UtilsService.formatCurrency(
                        FinanceService.getCostItemVat(data)
                      )
                    }}
                  </div>
                </template>
              ...
              >
                ...
                <template #body="{ data }">
                  <div class="text-center">
                    {{
                      UtilsService.formatCurrency(
                        FinanceService.getCostItemGross(data)
                      )
                    }}
                  </div>
                </template>
            ...
            <div
              class="invoice-items-summary flex justify-end mt-4 pt-4 border-t border-surface-500 dark:border-surface-400"
            >
        ...
        <template #footer>
          <div
            class="flex justify-end py-6 bg-surface-100 dark:bg-surface-800 rounded-xl"
          >
            <OfficeButton
              text="zapisz"
              class="mr-6"
