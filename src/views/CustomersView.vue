<script setup lang="ts">
      ...
      await customerStore
        ...
        .then(() => {
          toast.add({
            ...
            detail: 'Zaaktualizowano status klienta: ' + customerTemp.value?.firstName + ' ' + customerTemp.value?.name,
            life: 3000,
          });
        })
  ...
  const confirmDeleteInvoice = async (customer: Customer) => {
    customerTemp.value = customer;
    hasInvoices.value = await invoiceStore.getCustomerInvoices(customerTemp.value?.id).then((value) => {
  ...
  const submitDelete = async () => {
    ...
    if (customerTemp.value) {
      await customerStore
        .deleteCustomerDb(customerTemp.value.id)
        .then(() => {
          toast.add({
            severity: 'success',
            summary: 'Potwierdzenie',
...
<template>
  ...
  <Panel class="my-5 mx-2">
    ...
    <DataTable
      ...
      filter-display="menu"
      :global-filter-fields="['firstName', 'name', 'nip', 'address.street', 'address.city']"
    >
      <template #header>
      ...
      <template #empty>
        <h4 v-if="!customerStore.loadingCustomer" class="text-red-500">Nie znaleziono klientów...</h4>
      </template>
      ...
      <Column field="name" header="Nazwisko/Nazwa" :sortable="true" style="text-align: left">
      ...
      <Column header="Akcja" :exportable="false" style="min-width: 8rem">
        <template #body="slotProps">
          <div class="flex flex-row gap-1 justify-content-end">
            <OfficeIconButton class="text-orange-500" title="Edytuj klienta" icon="pi pi-file-edit" @click="editCustomer(slotProps.data)" />
            <OfficeIconButton class="text-red-500" title="Usuń klienta" icon="pi pi-trash" @click="confirmDeleteInvoice(slotProps.data)" />
          </div>
        </template>
      </Column>
      <template #expansion="slotProps">
        <div class="p-3">
          ...
          <p class="mt-2 ml-8" style="text-align: left"><b>Regon:</b> {{ slotProps.data.regon }}</p>
