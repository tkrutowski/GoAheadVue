...
export const useInvoiceStore = defineStore('invoice', {
  ...
  actions: {
    ...
    async getCustomerInvoices(customerId: number) {
      ...
      const result = this.invoices.filter((invoice: Invoice) => invoice.customer !== null && invoice.customer.id === customerId);
    ...
    getLatestItemForCustomer(customerId: number | undefined) {
      ...
      const itemsForId = this.invoices.filter((invoice) => invoice.customer?.id === customerId);
    ...
    async getPaymentType() {
      ...
      if (this.paymentTypes.length === 0) {
        ...
        const response = await httpCommon.get(`/goahead/invoice/paymenttype`).finally(() => (this.loadingPaymentType = false));
    ...
    async getStatistics() {
      ...
      this.loadingInvoices = true;
      const response = await httpCommon.get(`/goahead/invoice/status`);
    ...
    async getStatisticsByCustomer(year: number) {
      ...
      this.loadingInvoices = true;
      const response = await httpCommon.get(`/goahead/invoice/status/${year}`);
